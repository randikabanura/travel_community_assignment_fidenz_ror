                        ...
                        success: function (data2) {
                            ...
                            $.get("/api/v1/users/" + data[i].id + "/avatar_image_thumbnail", function (image, status) {
                                ...
                                $(".tab-pane#followers #followers_tab").append(\'<div class="single_user"><div class="card mb-3" style="min-width: 16rem;">\\n\' +
                                    \'  <div class="card-body text-dark image_card" style="display: flex; flex-direction: column;" data-userid="\'+ user_id + \'">\\n\' +
                                    ...
                                    \'    <hr style="width:150px; order:3"><button type="button" style="order:3" id=\' + (data[i].id) + \' class="btn btn-primary submit user_btn" data-remote="true" data-method="\'+ method +\'" data-url="\'+ url +\'" data-params="current_user_id=\' + user_id + \'&follow_user_id=\' + data[i].id + \'">\' + value + \'</button>\\n\' +
                                    \'  \\n\' +
                                    \'</div></div></div>\')
                            }).done( function(){
            ...
            $.each(data, function (i) {
                $.get("/api/v1/users/" + data[i].id + "/avatar_image_thumbnail", function (image, status) {
                    ...
                    $(".tab-pane#following #following_tab").append(\'<div class="single_user" id=\'+data[i].id+\'><div class="card  mb-3" style="min-width: 16rem;">\\n\' +
                        \'  <div class="card-body text-dark image_card" style="display: flex; flex-direction: column;" data-userid="\'+ user_id +\'">\\n\' +
                        \'     <hr style="width:210px; order:1"><div style="order: 2;align-self: center;padding: 0;"><img src="\' + link + \'" style="border-radius: 50%"/></div><a style="align-self:center" href="/profile/users/\' + data[i].id + \'" ><h5 class="card-title">\' + data[i].name + \'</h5></a>\\n\' +
                        \'   <hr style="width:150px; order:3"> <button type="button" style="order:3" id=\' + data[i].id + \' class="btn btn-primary submit user_btn" data-followingbtn="1" data-userid="\'+ user_id +\'" data-remote="true" data-method="\'+ method +\'" data-url="\'+url+\'" data-params="current_user_id=\' + user_id + \'&follow_user_id=\' + data[i].id + \'">Unfollow</button>\\n\' +
                        \'  \\n\' +
                        \'</div></div></div>\')
                })
            })
